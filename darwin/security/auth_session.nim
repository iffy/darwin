...
when not defined(__AuthSession__):
  const
    __AuthSession__* = 1
  ## !
  ## 	@header AuthSession
  ...
  const
    sessionIsRoot* = 0x00000001 ##  is the root session (startup/system programs)
    sessionHasGraphicAccess* = 0x00000010 ##  graphic subsystem (CoreGraphics et al) available
    sessionHasTTY* = 0x00000020 ##  /dev/tty is available
    sessionIsRemote* = 0x00001000 ##  session was established over the network
  ## !
  ...
  const
    sessionKeepCurrentBootstrap* = 0x00008000
  ## !
  ## 	@enum SessionStatus
  ...
  ##
  ##
  proc SessionGetInfo*(session: SecuritySessionId;
                      sessionId: ptr SecuritySessionId;
                      attributes: ptr SessionAttributeBits): OSStatus
  ## !
  ...
  ##
  ##
  proc SessionCreate*(flags: SessionCreationFlags; attributes: SessionAttributeBits): OSStatus