...
type
  SecRequirementRef* = ptr __SecRequirement
...
const
  ...
  kSecCSConsiderExpiration* = 1 shl 31 ##  consider expired certificates invalid
  kSecCSEnforceRevocationChecks* = 1 shl 30 ##  force revocation checks regardless of preference settings
  kSecCSNoNetworkAccess* = 1 shl 29 ##  do not use the network, cancels "kSecCSEnforceRevocationChecks"
  kSecCSReportProgress* = 1 shl 28 ##  make progress report call-backs when configured
  kSecCSCheckTrustedAnchors* = 1 shl 27 ##  build certificate chain to system trust anchors, not to any self-signed certificate
  kSecCSQuickCheck* = 1 shl 26    ##  (internal)
...
type
  SecCodeSignatureFlags* = uint32_t

const
  kSecCodeSignatureHost* = 0x00000001 ##  may host guest code
  kSecCodeSignatureAdhoc* = 0x00000002 ##  must be used without signer
  kSecCodeSignatureForceHard* = 0x00000100 ##  always set HARD mode on launch
  kSecCodeSignatureForceKill* = 0x00000200 ##  always set KILL mode on launch
  kSecCodeSignatureForceExpiration* = 0x00000400 ##  force certificate expiration checks
  kSecCodeSignatureRestrict* = 0x00000800 ##  restrict dyld loading
  kSecCodeSignatureEnforcement* = 0x00001000 ##  enforce code signing
  kSecCodeSignatureLibraryValidation* = 0x00002000 ##  library validation required
  kSecCodeSignatureRuntime* = 0x00010000 ##  apply runtime hardening policies
...
const
  ...
  kSecInvalidRequirementType* = 6 ##  invalid type of Requirement (must be last)
