...
when TARGET_OS_OSX:
  when defined(SEC_IOS_ON_OSX):
    const
      SEC_OS_IPHONE* = 1
      SEC_OS_OSX* = 0
      SEC_OS_OSX_INCLUDES* = 1
when TARGET_OS_MACCATALYST:
  const
    SEC_OS_IPHONE* = 1
    SEC_OS_OSX* = 0
    SEC_OS_OSX_INCLUDES* = 0
when not defined(SEC_OS_IPHONE):
...
when defined(__clang__):
  ...
  const
    SEC_DEPRECATED_ATTRIBUTE* = true
const
  SECURITY_TYPE_UNIFICATION* = 1
...
type
  SecCertificateRef* = ptr __SecCertificate

when TARGET_OS_OSX:
  type
    OpaqueSecCertificateRef* = __SecCertificate
## !
...
type
  SecIdentityRef* = ptr __SecIdentity

when TARGET_OS_OSX:
  type
    OpaqueSecIdentityRef* = __SecIdentity
## !
##     @typedef SecKeyRef
...
type
  SecKeyRef* = ptr __SecKey

when TARGET_OS_OSX:
  type
    OpaqueSecKeyRef* = __SecKey
## !
...
type
  SecAccessControlRef* = ptr __SecAccessControl
...
type
  SecKeychainRef* = ptr __SecKeychain
...
type
  SecKeychainItemRef* = ptr __SecKeychainItem
...
type
  SecKeychainSearchRef* = ptr __SecKeychainSearch
...
type
  SecKeychainAttribute* {.bycopy.} = object
...
type
  SecKeychainAttributePtr* = ptr SecKeychainAttribute
...
type
  SecKeychainAttributeList* {.bycopy.} = object
    count*: UInt32
    attr*: ptr SecKeychainAttribute
...
type
  SecTrustedApplicationRef* = ptr __SecTrustedApplication
...
type
  SecAccessRef* = ptr __SecAccess

when TARGET_OS_OSX:
  type
    OpaqueSecAccessRef* = __SecAccess
## !
##     @typedef SecACLRef
...
type
  SecPasswordRef* = ptr __SecPassword
...
type
  SecKeychainAttributeInfo* {.bycopy.} = object
    count*: UInt32
    tag*: ptr UInt32
    format*: ptr UInt32
...
proc SecCopyErrorMessageString*(status: OSStatus; reserved: pointer): CFStringRef
## !
